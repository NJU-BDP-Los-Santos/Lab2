...
public class Main {
    public static void main(String[] args) throws Exception
    {
        try {
            ...
            job.setReducerClass(Main.MergeReducer.class);
            job.setCombinerClass(Main.CombinerSameWordDoc.class);
            job.setPartitionerClass(Main.DividePartitioner.class);
...
//                context.write(word, new Text(fileName));
    ...
    public static class MergeReducer extends Reducer<Text, IntWritable, Text, Text>
    {
        ...
        protected void setup(Context context)
        {
            ...
            worddoc_count = 0;
            output_ = new String();
            words_sum = new HashMap<String, Double>();
            doc_sum = 0.0;
        }
        @Override
        protected void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException
        {
            ...
            String auther = key.toString().split("#")[1].split("\\d+")[0];
//            System.out.println(t);
//            System.out.println(t_prev);
            if (!t.equals(t_prev) && t_prev != null && !t_prev.equals(""))
            {
                Set<Map.Entry<String, Double>> set =words_sum.entrySet();
                Iterator<Map.Entry<String, Double>> itera_Entry =set.iterator();
            ...
            if(!words_sum.containsKey(auther))
            {
                words_sum.put(auther, 0.0);
            }
            Double cur_sum =  words_sum.get(auther) + (double)count;
            words_sum.put(auther,cur_sum);
            doc_sum += 1.0;
            t_prev = t;
